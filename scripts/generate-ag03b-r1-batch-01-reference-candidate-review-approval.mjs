...
const configPath = path.join(root, "data", "quality", "ag03b-r1-batch-01-reference-candidate-review-approval.json");
...
const ag03b = readJson(path.join(root, config.input_files.ag03b_candidate_registry));
const policy = config.approval_policy;

const entries = (ag03b.entries || []).map((entry, index) => ({
  approval_id: `AG03B_R1_APPROVAL_${String(index + 1).padStart(3, "0")}`,
  ...
  approved_for_ag03c_insertion: true,
...
const approval = {
  approval_record_id: "AG03B_R1_BATCH_01_REFERENCE_CANDIDATE_APPROVAL_RECORD",
  module_id: "AG03B-R1",
  status: "batch_01_reference_candidates_approved_for_ag03c_insertion",
  generated_at: new Date().toISOString(),
  source_candidate_registry: config.input_files.ag03b_candidate_registry,
  ...
  summary: {
    ...
    article_reference_insertion_performed: false,
    ready_for_ag03c: true
  },
  entries
};

const preview = {
  preview_id: "AG03B_R1_BATCH_01_REFERENCE_CANDIDATE_APPROVAL_PREVIEW",
  module_id: "AG03B-R1",
  status: "preview_batch_01_reference_candidate_approval",
  preview_only: true,
...
console.log(`Ready for AG03C: ${approval.summary.ready_for_ag03c}`);
