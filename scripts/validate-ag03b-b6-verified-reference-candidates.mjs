...
const docPath = path.join(root, "docs", "quality", "AG03B_B6_VERIFIED_REFERENCE_CANDIDATE_POPULATION.md");
const candidatePath = path.join(root, "data", "editorial", "ag03b-b6-verified-reference-candidates.json");
const previewPath = path.join(root, "data", "quality", "ag03b-b6-verified-reference-candidates-preview.json");
const ag03aPath = path.join(root, "data", "editorial", "ag03a-verified-reference-scaling-readiness-queue.json");
const ag03dB5Path = path.join(root, "data", "editorial", "ag03d-b5-post-ag03c-b5-reference-insertion-audit.json");
const packagePath = path.join(root, "package.json");

function fail(message) {
  console.error(`❌ AG03B-B6 validation failed: ${message}`);
  process.exit(1);
}
...
for (const p of [registryPath, docPath, candidatePath, previewPath, ag03aPath, ag03dB5Path, packagePath]) {
  if (!fs.existsSync(p)) fail(`Missing AG03B-B6 required file: ${p}`);
}
...
const ag03dB5 = readJson(ag03dB5Path);
const pkg = readJson(packagePath);
const docText = fs.readFileSync(docPath, "utf8");

if (config.module_id !== "AG03B-B6") fail("Registry module_id must be AG03B-B6");
if (candidates.module_id !== "AG03B-B6") fail("Candidate registry module_id must be AG03B-B6");
if (preview.module_id !== "AG03B-B6") fail("Preview module_id must be AG03B-B6");
if (preview.preview_only !== true) fail("Preview must be preview-only");

if (ag03dB5.summary?.ready_for_ag03b_batch_6 !== true) fail("AG03D-B5 must authorize Batch 6");

const batch = ag03a.batches.find((b) => b.batch_id === config.target_batch_id);
if (!batch) fail("AG03A target Batch 6 must exist");

if (candidates.summary.batch_article_count !== config.required_articles) fail("AG03B-B6 must populate exactly required batch articles");
if (candidates.summary.batch_article_count !== batch.article_count) fail("AG03B-B6 article count must match AG03A Batch 6 count");
if (candidates.summary.total_candidate_reference_count !== config.required_articles * config.required_references_per_article) {
  fail("AG03B-B6 must record exactly 24 candidate references");
}
if (candidates.summary.entries_with_two_candidates !== config.required_articles) fail("Every article must have two candidates");
if (candidates.summary.approved_for_article_insertion_count !== 0) fail("No article insertion approval in AG03B-B6");
if (candidates.summary.article_reference_insertion_performed !== false) fail("Article insertion must remain false");

for (const entry of candidates.entries) {
  if (!batch.article_paths.includes(entry.article_path)) fail(`Entry is not in AG03A Batch 6: ${entry.article_path}`);
  ...
  if (entry.article_insertion_status !== "not_inserted_in_ag03b_b6") fail(`Entry insertion status invalid: ${entry.article_path}`);
  ...
  for (const ref of entry.references) {
    ...
    if (ref.article_insertion_status !== "not_inserted_in_ag03b_b6") fail(`Reference insertion status invalid: ${entry.article_path}`);
...
if (candidates.reference_candidate_population_performed !== true) fail("Reference candidate population must be true for AG03B-B6");
...
if (config.reference_candidate_population_enabled !== true) fail("AG03B-B6 candidate population flag must be true");
if (config.operator_external_research_used !== true) fail("AG03B-B6 must record operator external research");

for (const scriptName of ["generate:ag03b-b6", "validate:ag03b-b6", "validate:project"]) {
  if (!pkg.scripts?.[scriptName]) fail(`Missing package script: ${scriptName}`);
}

for (const phrase of ["Purpose", "Scope", "Source Selection Rule", "Explicit Exclusions", "Acceptance Criteria"]) {
  if (!docText.includes(phrase)) fail(`AG03B-B6 document missing phrase: ${phrase}`);
}

pass("AG03B-B6 registry is present.");
pass("AG03B-B6 document is present.");
pass("AG03B-B6 candidate registry and preview outputs are present.");
pass("AG03D-B5 authorization for Batch 6 is verified.");
pass("Exactly 12 Batch 6 articles are populated.");
...
pass("AG03B-B6 candidate population is safe to commit.");
