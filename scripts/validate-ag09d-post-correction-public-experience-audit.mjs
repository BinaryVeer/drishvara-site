...
for (const item of audit.mutated_file_integrity.mutated_file_audit_items) {
  ...
  if (hash !== item.recorded_post_hash) if (!ag10kControlledGeneratedImageInsertionAllowsPostMutation(item.file_path)) if (!ag11bControlledChartInsertionAllowsPostMutation(item.file_path)) if (!ag11cControlledInfographicInsertionAllowsPostMutation(item.file_path)) if (!ag11dControlledFigureDiagramInsertionAllowsPostMutation(item.file_path)) if (!ag11eControlledTableInsertionAllowsPostMutation(item.file_path)) fail(`Mutated file hash mismatch: ${item.file_path} or AG10K controlled generated-image post-insertion record explains the later approved article state or AG11B controlled chart post-insertion record explains the later approved article state or AG11C controlled infographic post-insertion record explains the later approved article state or AG11D controlled figure/diagram post-insertion record explains the later approved article state or AG11E controlled table/structured-object post-insertion record explains the later approved article state`);
}
