...
const configPath = path.join(root, "data", "quality", "ag03b-b6-r1-reference-candidate-review-approval.json");
...
const candidates = readJson(path.join(root, config.input_files.ag03b_b6_candidate_registry));
const policy = config.approval_policy;

if (candidates.module_id !== "AG03B-B6") {
  throw new Error("Input candidate registry must be AG03B-B6.");
}

const entries = (candidates.entries || []).map((entry, index) => ({
  approval_id: `AG03B_B6_R1_APPROVAL_${String(index + 1).padStart(3, "0")}`,
  batch_id: policy.target_batch_id,
  article_path: entry.article_path,
  category: entry.category,
  title: entry.title,
  source_selection_key: entry.source_selection_key || null,
  approved_reference_count: entry.references.length,
  approval_status: policy.approval_status,
  article_insertion_status: policy.article_insertion_status,
  approved_for_ag03c_b6_insertion: true,
  references: entry.references.map((ref) => ({
    ...
    source_selection_key: ref.source_selection_key || entry.source_selection_key || null,
    approval_status: policy.approval_status,
    article_insertion_status: policy.article_insertion_status
  }))
}));
...
const approval = {
  approval_record_id: "AG03B_B6_R1_REFERENCE_CANDIDATE_APPROVAL_RECORD",
  module_id: "AG03B-B6-R1",
  status: "batch_06_reference_candidates_approved_for_ag03c_b6_insertion",
  generated_at: new Date().toISOString(),
  source_candidate_registry: config.input_files.ag03b_b6_candidate_registry,
  ...
  summary: {
    ...
    article_reference_insertion_performed: false,
    ready_for_ag03c_b6: true
  },
  entries
};

const preview = {
  preview_id: "AG03B_B6_R1_REFERENCE_CANDIDATE_APPROVAL_PREVIEW",
  module_id: "AG03B-B6-R1",
  status: "preview_batch_06_reference_candidate_approval",
  preview_only: true,
...
console.log(`Ready for AG03C-B6: ${approval.summary.ready_for_ag03c_b6}`);
