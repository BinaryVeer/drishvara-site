...
const docPath = path.join(root, "docs", "quality", "AG03B_BATCH_01_VERIFIED_REFERENCE_CANDIDATE_POPULATION.md");
const candidatePath = path.join(root, "data", "editorial", "ag03b-batch-01-verified-reference-candidates.json");
const previewPath = path.join(root, "data", "quality", "ag03b-batch-01-verified-reference-candidates-preview.json");
const ag03aPath = path.join(root, "data", "editorial", "ag03a-verified-reference-scaling-readiness-queue.json");
const packagePath = path.join(root, "package.json");

function fail(message) {
  console.error(`❌ AG03B validation failed: ${message}`);
  process.exit(1);
}
...
for (const p of [registryPath, docPath, candidatePath, previewPath, ag03aPath, packagePath]) {
  if (!fs.existsSync(p)) fail(`Missing AG03B required file: ${p}`);
}
...
if (config.module_id !== "AG03B") fail("Registry module_id must be AG03B");
if (candidates.module_id !== "AG03B") fail("Candidate registry module_id must be AG03B");
if (preview.module_id !== "AG03B") fail("Preview module_id must be AG03B");
...
if (!batch) fail("AG03A target batch must exist");

if (candidates.summary.batch_article_count !== config.required_articles) fail("AG03B must populate exactly required batch articles");
if (candidates.summary.batch_article_count !== batch.article_count) fail("AG03B article count must match AG03A batch count");
if (candidates.summary.total_candidate_reference_count !== config.required_articles * config.required_references_per_article) {
  fail("AG03B must record exactly 24 candidate references");
}
if (candidates.summary.entries_with_two_candidates !== config.required_articles) fail("Every article must have two candidates");
if (candidates.summary.sample_article_count !== 0) fail("AG03B Batch 1 must not include AR02B sample articles");
if (candidates.summary.approved_for_article_insertion_count !== 0) fail("No article insertion approval in AG03B");
if (candidates.summary.article_reference_insertion_performed !== false) fail("Article insertion must remain false");

for (const entry of candidates.entries) {
  if (!batch.article_paths.includes(entry.article_path)) fail(`Entry is not in AG03A Batch 1: ${entry.article_path}`);
  ...
  if (entry.article_insertion_status !== "not_inserted_in_ag03b") fail(`Entry insertion status invalid: ${entry.article_path}`);
  ...
  for (const ref of entry.references) {
    ...
    if (ref.article_insertion_status !== "not_inserted_in_ag03b") fail(`Reference insertion status invalid: ${entry.article_path}`);
...
if (candidates.reference_candidate_population_performed !== true) fail("Reference candidate population must be true for AG03B");
...
if (config.reference_candidate_population_enabled !== true) fail("AG03B candidate population flag must be true");
if (config.operator_external_research_used !== true) fail("AG03B must record operator external research");

for (const scriptName of ["generate:ag03b", "validate:ag03b", "validate:project"]) {
  if (!pkg.scripts?.[scriptName]) fail(`Missing package script: ${scriptName}`);
}

for (const phrase of ["Purpose", "Scope", "Source Selection Rule", "Explicit Exclusions", "Acceptance Criteria"]) {
  if (!docText.includes(phrase)) fail(`AG03B document missing phrase: ${phrase}`);
}

pass("AG03B registry is present.");
pass("AG03B document is present.");
pass("AG03B candidate registry and preview outputs are present.");
pass("Exactly 12 Batch 1 articles are populated.");
...
pass("AG03B Batch 1 candidate population is safe to commit.");
