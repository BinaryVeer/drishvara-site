...
const configPath = path.join(root, "data", "quality", "ag03c-b5-article-reference-insertion.json");
...
function stripAg03cB3(html) {
  html = html.replace(/<section\b[^>]*data-drishvara-ag03c-b5-reference-block="true"[^>]*>[\s\S]*?<\/section>\s*/gi, "");
  html = html.replace(/<meta\b[^>]*data-drishvara-ag03c-b5-checked="true"[^>]*>\s*/gi, "");
  return html;
}

function markChecked(html) {
  if (html.includes('data-drishvara-ag03c-b5-checked="true"')) return html;
  const marker = '<meta data-drishvara-ag03c-b5-checked="true" content="batch-05-approved-references-inserted">';
...
function referenceBlock(entry) {
  const links = entry.references.map((ref) => {
    return `<li>
      <a data-drishvara-ag03c-b5-reference-link="true" href="${escapeAttr(ref.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(ref.title)}</a>
      <span class="drishvara-reference-source"> — ${escapeHtml(ref.publisher)}</span>
    </li>`;
  }).join("\n");

  return `<section class="drishvara-article-evidence" data-drishvara-ag03c-b5-reference-block="true" data-drishvara-ag03c-batch="03">
  <h2>References</h2>
  <p>Verified references approved under AG03B-B5-R1 for Batch 5 insertion.</p>
  <ol>
${links}
  </ol>
</section>
...
for (const entry of approval.entries || []) {
  ...
  if (!fs.existsSync(full)) throw new Error(`Approved Batch 5 article page missing: ${rel}`);
...
const applyResult = {
  apply_id: "AG03C_B5_ARTICLE_REFERENCE_INSERTION_APPLY_RESULT",
  module_id: "AG03C-B5",
  status: "batch_05_approved_references_inserted",
  applied: true,
  ...
  summary: {
    ...
    articles_with_two_ag03c_b5_reference_links: fileResults.filter((row) => row.ag03c_b5_reference_link_count === 2).length,
    every_article_has_one_ag03c_b5_block: fileResults.every((row) => row.ag03c_b5_reference_block_count === 1),
    every_article_has_checked_marker: fileResults.every((row) => row.ag03c_b5_checked_present),
...
const preview = {
  preview_id: "AG03C_B5_ARTICLE_REFERENCE_INSERTION_PREVIEW",
  module_id: "AG03C-B5",
  status: "preview_after_batch_05_reference_insertion",
  preview_only: true,
  summary: applyResult.summary,
  sample_entries: fileResults.slice(0, 12).map((row) => ({
    article_path: row.article_path,
    ag03c_b5_reference_link_count: row.ag03c_b5_reference_link_count,
...
console.log(`Articles with two AG03C-B5 links: ${applyResult.summary.articles_with_two_ag03c_b5_reference_links}`);
