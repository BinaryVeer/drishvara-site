...
const configPath = path.join(root, "data", "quality", "ag03c-batch-01-article-reference-insertion.json");
...
function stripAg03c(html) {
  html = html.replace(/<section\b[^>]*data-drishvara-ag03c-reference-block="true"[^>]*>[\s\S]*?<\/section>\s*/gi, "");
  html = html.replace(/<meta\b[^>]*data-drishvara-ag03c-checked="true"[^>]*>\s*/gi, "");
  return html;
}

function markChecked(html) {
  if (html.includes('data-drishvara-ag03c-checked="true"')) return html;
  const marker = '<meta data-drishvara-ag03c-checked="true" content="batch-01-approved-references-inserted">';
...
function referenceBlock(entry) {
  const links = entry.references.map((ref) => {
    return `<li>
      <a data-drishvara-ag03c-reference-link="true" href="${escapeAttr(ref.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(ref.title)}</a>
      <span class="drishvara-reference-source"> — ${escapeHtml(ref.publisher)}</span>
    </li>`;
  }).join("\n");

  return `<section class="drishvara-article-evidence" data-drishvara-ag03c-reference-block="true" data-drishvara-ag03c-batch="01">
  <h2>References</h2>
  <p>Verified references approved under AG03B-R1 for Batch 1 insertion.</p>
  <ol>
${links}
  </ol>
</section>
...
function insertReferenceBlock(html, entry) {
  ...
  const ar02cMatch = html.match(/<section\b[^>]*data-drishvara-ar02c-sample-reference-block="true"[^>]*>[\s\S]*?<\/section>/i);
  if (ar02cMatch && typeof ar02cMatch.index === "number") {
    return html.slice(0, ar02cMatch.index) + block + html.slice(ar02cMatch.index);
  }
...
for (const entry of approval.entries || []) {
  ...
  if (!fs.existsSync(full)) throw new Error(`Approved article page missing: ${rel}`);

  const before = fs.readFileSync(full, "utf8");
  const beforeWithoutAg03c = stripAg03c(before);

  let html = insertReferenceBlock(beforeWithoutAg03c, entry);
  ...
  fileResults.push({
    ...
    before_ag02_hero_present: beforeWithoutAg03c.includes('data-drishvara-ag02-hero-visual="true"'),
    after_ag02_hero_present: html.includes('data-drishvara-ag02-hero-visual="true"'),
    ag02_hero_preserved: !beforeWithoutAg03c.includes('data-drishvara-ag02-hero-visual="true"') || html.includes('data-drishvara-ag02-hero-visual="true"'),

    before_ag02_credit_present: beforeWithoutAg03c.includes('data-drishvara-ag02-image-credit="true"'),
    after_ag02_credit_present: html.includes('data-drishvara-ag02-image-credit="true"'),
    ag02_credit_preserved: !beforeWithoutAg03c.includes('data-drishvara-ag02-image-credit="true"') || html.includes('data-drishvara-ag02-image-credit="true"'),
...
const applyResult = {
  apply_id: "AG03C_BATCH_01_ARTICLE_REFERENCE_INSERTION_APPLY_RESULT",
  module_id: "AG03C",
  status: "batch_01_approved_references_inserted",
  applied: true,
  ...
  summary: {
    ...
    articles_with_two_ag03c_reference_links: fileResults.filter((row) => row.ag03c_reference_link_count === 2).length,
    every_article_has_one_ag03c_block: fileResults.every((row) => row.ag03c_reference_block_count === 1),
    every_article_has_checked_marker: fileResults.every((row) => row.ag03c_checked_present),
...
const preview = {
  preview_id: "AG03C_BATCH_01_ARTICLE_REFERENCE_INSERTION_PREVIEW",
  module_id: "AG03C",
  status: "preview_after_batch_01_reference_insertion",
  preview_only: true,
  summary: applyResult.summary,
  sample_entries: fileResults.slice(0, 12).map((row) => ({
    article_path: row.article_path,
    ag03c_reference_link_count: row.ag03c_reference_link_count,
...
console.log(`Articles with two AG03C links: ${applyResult.summary.articles_with_two_ag03c_reference_links}`);
console.log(`All URLs match approval: ${applyResult.summary.every_article_matches_approval_urls}`);
console.log(`AG02 hero preserved: ${applyResult.summary.every_article_preserves_ag02_hero}`);
console.log(`AG02 credit preserved: ${applyResult.summary.every_article_preserves_ag02_credit}`);
