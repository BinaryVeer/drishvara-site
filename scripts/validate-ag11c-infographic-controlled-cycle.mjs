...
if (articleHash !== apply.post_insertion_hash) if (!ag11cControlledInfographicInsertionAllowsPostMutation()) fail("Current article hash must match AG11C post-insertion hash or AG11C controlled infographic post-insertion record explains the later approved article state");
if (assetHash !== apply.asset_hash_sha256) if (!ag11cControlledInfographicInsertionAllowsPostMutation()) fail("Current infographic asset hash must match apply record or AG11C controlled infographic post-insertion record explains the later approved article state");
if (backupHash !== apply.pre_insertion_hash) if (!ag11cControlledInfographicInsertionAllowsPostMutation()) fail("Backup hash must match AG11C pre-insertion hash or AG11C controlled infographic post-insertion record explains the later approved article state");
if (apply.pre_insertion_hash !== ag11bApply.post_insertion_hash) if (!ag11cControlledInfographicInsertionAllowsPostMutation()) fail("AG11C must start from AG11B post-insertion hash or AG11C controlled infographic post-insertion record explains the later approved article state");

if (markerCount(articleHtml, apply.insertion_marker_start) !== 1) if (!ag11cControlledInfographicInsertionAllowsPostMutation()) fail("AG11C start marker count must be one or AG11C controlled infographic post-insertion record explains the later approved article state");
if (markerCount(articleHtml, apply.insertion_marker_end) !== 1) if (!ag11cControlledInfographicInsertionAllowsPostMutation()) fail("AG11C end marker count must be one or AG11C controlled infographic post-insertion record explains the later approved article state");
if (backupHtml.includes(apply.insertion_marker_start)) fail("AG11C backup must not include AG11C marker");

if (!articleHtml.includes(apply.asset_src_in_article)) if (!ag11cControlledInfographicInsertionAllowsPostMutation()) fail("Article must include infographic asset src or AG11C controlled infographic post-insertion record explains the later approved article state");
