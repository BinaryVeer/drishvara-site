...
for (const [relative, expected] of Object.entries(preserved)) {
  if (sha(relative) !== expected) fail(`AG74P public baseline changed: ${relative}`);
}

const closure = JSON.parse(
  fs.readFileSync(
    full("data/content-intelligence/quality-registry/ag74p-two-asset-final-closure-record.json"),
    "utf8"
  )
);
if (
...
const packageJson = JSON.parse(fs.readFileSync(full("package.json"), "utf8"));
for (const script of [
...
if (payloads.runtime_releases.length !== 1) fail("One staged runtime release is required.");
if (payloads.aliases.length < 5) fail("Normalized alias projection is incomplete.");
if (new Set(payloads.aliases.map((item) => `${item.alias_normalized}:${item.canonical_place_id}`)).size !== payloads.aliases.length) {
  fail("Normalized aliases contain duplicates.");
}

const boundary = JSON.parse(
  fs.readFileSync(
    full("data/content-intelligence/mutation-plans/sup01-single-stage-execution-boundary.json"),
    "utf8"
  )
);
if (
  ...
  fail("SUP01 mutation boundary mismatch.");
}
...
].map((relative) => fs.readFileSync(full(relative), "utf8")).join("\n");

if (
  /eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+|sb_secret_|postgres(?:ql)?:\/\//.test(allText)
) {
