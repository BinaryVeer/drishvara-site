...
const configPath = path.join(root, "data", "quality", "ag03b-b4-r1-reference-candidate-review-approval.json");
...
const candidates = readJson(path.join(root, config.input_files.ag03b_b4_candidate_registry));
const policy = config.approval_policy;

if (candidates.module_id !== "AG03B-B4") {
  throw new Error("Input candidate registry must be AG03B-B4.");
}

const entries = (candidates.entries || []).map((entry, index) => ({
  approval_id: `AG03B_B4_R1_APPROVAL_${String(index + 1).padStart(3, "0")}`,
  ...
  approved_for_ag03c_b4_insertion: true,
...
const approval = {
  approval_record_id: "AG03B_B4_R1_REFERENCE_CANDIDATE_APPROVAL_RECORD",
  module_id: "AG03B-B4-R1",
  status: "batch_04_reference_candidates_approved_for_ag03c_b4_insertion",
  generated_at: new Date().toISOString(),
  source_candidate_registry: config.input_files.ag03b_b4_candidate_registry,
  ...
  summary: {
    ...
    article_reference_insertion_performed: false,
    ready_for_ag03c_b4: true
  },
  entries
};

const preview = {
  preview_id: "AG03B_B4_R1_REFERENCE_CANDIDATE_APPROVAL_PREVIEW",
  module_id: "AG03B-B4-R1",
  status: "preview_batch_04_reference_candidate_approval",
  preview_only: true,
...
console.log(`Ready for AG03C-B4: ${approval.summary.ready_for_ag03c_b4}`);
